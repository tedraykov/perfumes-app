...
export class ParfiumbgScraper {
	...
	async extractPerfumeDetails(url: string, data: PerfumeData) {
		try {
	...
	async processor({ perfume, inventory }: PerfumeData) {
		...
		await db.insert(houses).values({ name: perfume.house }).onConflictDoNothing().execute();
		...
		const [upserted] = await db
			...
			.values(perfume)
			.onConflictDoUpdate({
				target: [perfumes.name, perfumes.house, perfumes.gender, perfumes.concentration],
				set: {
					image_url: perfume.image_url,
					fragrantica_url: perfume.fragrantica_url
				}
			})
			.returning({ id: perfumes.id });
		if (!upserted?.id) {
		...
		// 2) For each inventory item: look up the website, then upsert inventory
		for (const item of inventory) {
			const website = await db
				.select()
				.from(websites)
				.where(eq(websites.name, item.website || ''))
				.limit(1)
				.then((rows) => rows[0]);
			if (!website) {
				throw new Error('add website before scraping invenotry');
			}
			...
			await db
				...
				.onConflictDoUpdate({
					target: [
						inventorySchema.perfume_id,
						inventorySchema.website,
						inventorySchema.volume,
						inventorySchema.is_tester
					],
					set: {
						price: item.price,
						url: item.url
